...
def get_args():
    ...
    parser.add_argument('--model', '-m', type=str, default='', help='select any model from a,b,c,d,e')
...
def test_model(best_model_a, best_model_b, best_model_c, best_model_d, best_model_e, test_dataset):
    """
    Tests multiple models on the given test dataset and computes the accuracy using majority voting.
    ...
    for iterations, (images, labels) in enumerate(tqdm(test_dataset, desc="Testing")):
        ...
        with torch.no_grad():
            ...
            # feature merging
            combined_predictions = torch.stack(
                [predictions_a, predictions_b, predictions_c, predictions_d, predictions_e])
    ...
    print(f'Accuracy of Decision Level Fusion(majority voting): {accuracy:.4f}')
