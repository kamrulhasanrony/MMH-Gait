...
def test_model(best_model_a, best_model_b, best_model_c, best_model_d, best_model_e, test_dataset):
    """
    Tests multiple models on the given test dataset and computes the accuracy using hybrid fusion.
    ...
    for iterations, (images, labels) in enumerate(tqdm(test_dataset, desc="Testing")):
        ...
        with torch.no_grad():
            ...
            predictions_a = torch.argmax(outputs_a.cpu(), dim=1)
            # for model b
            outputs_b = best_model_b(images)
            predictions_b = torch.argmax(outputs_b.cpu(), dim=1)

            # for model c
            outputs_c = best_model_c(images)
            predictions_c = torch.argmax(outputs_c.cpu(), dim=1)

            # for model d
            outputs_d = best_model_c(images)
            predictions_d = torch.argmax(outputs_d.cpu(), dim=1)

            # for model e
            outputs_e = best_model_c(images)
            predictions_e = torch.argmax(outputs_e.cpu(), dim=1)

            # feature merging
            merged_feature = (outputs_b + outputs_c) / 2.0
            predictions_merged = torch.argmax(merged_feature.cpu(), dim=1)

            # feature merging
            merged_feature2 = (outputs_d + outputs_e) / 2.0
            predictions_merged2 = torch.argmax(merged_feature2.cpu(), dim=1)

            # output merging
            combined_predictions = torch.stack([predictions_merged, predictions_merged2, predictions_a])
            combined_predictions = combined_predictions.t()

            def majority_vote(predictions):
                return Counter(predictions.numpy()).most_common(1)[0][0]

            final_predictions = [majority_vote(instance_predictions) for instance_predictions in combined_predictions]
            all_predictions_merged.extend(final_predictions)
    ...
    print(f'Accuracy of Hybrid Fusion: {accuracy:.4f}')
